...
func TestNodePackConformance(t *testing.T) {
	...
	request := contracts.NodeInvocationRequest{SchemaVersion: protocol.InvocationSchemaVersion, InvocationID: "inv-1", RunID: "run-1", NodeID: "fleet", TypeRef: executor.Descriptor().TypeRef, DescriptorDigest: executor.Descriptor().DescriptorDigest, AttemptID: "att-1", AttemptOrdinal: 1, Input: input, InputDigest: digest, RequestedAt: t0, Deadline: t0.Add(time.Minute)}
